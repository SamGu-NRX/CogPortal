...
#:
#: 10 (2026-09-03): every pairing trial gets its own store object rather than
#: sharing the one instance the scan built, so a pairing an earlier trial's
#: leftovers had made raise is now reachable and a 9 entry can name a worse
#: pairing than the search would pick today. A store's pre-existing tables no
#: longer count toward the state form being ambiguous. And `attemptsTried` is
#: now the whole search rather than the accepted chain's own ordinal.
FORMAT = 10
...
def fingerprint(paths: Sequence[Path], *, benchmark: str) -> str:
    ...
    for path in sorted(Path(p) for p in paths):
        ...
        try:
            ...
            # change, so recording that it could not be read makes the key
...
def source_paths(discovery: Any) -> List[Path]:
    ...
    exactly the change that should invalidate the cache.
    """
