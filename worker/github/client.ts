import { FIXTURE_REPO, FIXTURE_SCENARIOS } from "@shared/fixtures";
...
function parseRepository(value: unknown): GitHubRepository {
  if (!isRecord(value) || !isRecord(value.owner) || typeof value.private !== "boolean") {
  ...
  async listRepositories(token: string): Promise<GitHubRepositoryListing[]> {
    ...
    return Promise.all(
      repositories.map(async (repo) => {
        ...
        const branchNames = payload.map((value) => {
