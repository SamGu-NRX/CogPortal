...
import { JoinCohortRequestSchema, SessionSchema } from "@shared/schema";
...
export function registerCohortRoutes(app: Hono<AppEnv>): void {
  app.post("/cohorts/:slug/join", async (c) => {
    ...
    const [cohort] = await db
      .select()
      .from(cohorts)
      .where(and(eq(cohorts.slug, c.req.param("slug")), eq(cohorts.active, true)))
      .limit(1);
    if (!cohort) throw new ApiHttpError(404, "not_found", "Active cohort not found.");
    if (cohort.joinCode !== body.code) {
      throw new ApiHttpError(403, "cohort_code_invalid", "The cohort join code is invalid.");
    }
    await db.update(users).set({ cohortId: cohort.id }).where(eq(users.id, auth.user.id));
