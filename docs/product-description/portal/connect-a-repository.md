...
The screen has one property worth naming up front: it is the last point at which a student is still an individual. Everything before it is about them, and everything after it is about their team. Nothing on the choice screen says so. It carries a heading, "Set up your team" (`apps/portal/src/routes/ConnectPage.tsx:97`), and two cards, and the closest it comes to naming the consequence is the second card's hint: "Fork the course template and connect your fork. You'll be the team's creator." (`:107`). That the repository is the team is enforced by the indexes below and stated nowhere on this screen.
...
Nothing else sits above the list. The picker, or the empty state that replaces it, is the first thing under the heading and its one line, and the screen offers no advice about where a fork should live. An organization is never mentioned on this screen.

### Answered without work

**No repositories.** The list comes back empty and a card replaces it, headed "No repositories are visible yet. Three short steps:" (`apps/portal/src/routes/ConnectPage.tsx:442`). Step 01 says "Fork {template}. Keep it public." next to a button labelled "Fork {template}" that opens the template's fork page in a new tab (`:448`). Step 02 says "Install the app on the account that owns the fork." with the access link under it (`:475`). Step 03 says "It shows up here." next to a "Check again" button that refetches (`:482`).
...
The team name field stays editable throughout. Editing it while the request is in flight has no effect on the request, which captured the trimmed value when it was built (`apps/portal/src/routes/ConnectPage.tsx:357`), and the edited value is what a retry would send.

Nothing else on the page is disabled. The access link under the picker still opens GitHub in a new tab, which is harmless and occasionally useful: a student whose connect is about to fail on the fork check can go and look at the parent repository while they wait. The fork button is not one of the things they can press, because it belongs to the empty state, and the empty state is gone by the time a repository has been selected.

### How it ends

On success the portal returns a fresh session and the page navigates to `/setup` with `replace` and nothing else (`apps/portal/src/routes/ConnectPage.tsx:358`). That is the same navigation whether the student created the team or attached themselves to a repository another team already held, and the "Join a team" list performs it too (`:206`), so the setup guide cannot tell which of the three ways in a student took and does not try: it reads no router state at all.
...
On failure the sentence appears above the button. The server's own message when the error is an `ApiRequestError`, and "Connecting failed. Try again." otherwise (`apps/portal/src/routes/ConnectPage.tsx:404`). The selection and the typed name are kept.
...
| Practice or leaderboard | No effect on the ask. The team name typed here is the name that appears on the public leaderboard, and the field says so: "Shown on the public leaderboard. You can rename it later." (`apps/portal/src/routes/ConnectPage.tsx:397`). | No effect. |
| Flags, options, and where you are typing | `templateRepo` fills the template's name into three places, and each handles an unset name differently. The "Start a team" subtitle falls back to the words "the course template" (`apps/portal/src/routes/ConnectPage.tsx:128`), step 01 of the empty state falls back to the same words (`:448`), and the fork button does not render at all, because it is the one of the three that needs a URL rather than a name (`:450`). `appSlug` decides whether the access link renders at all (`apps/portal/src/components/GrantAccess.tsx:18`). `GITHUB_TEMPLATE_REPO_ID` decides whether the fork check compares ids or names. The team name field caps at 60 characters, and so does the contract (`packages/contracts/src/schema.ts:1009`). | No effect. |
...
The one interrupt worth planning for is the tab closed between the server's writes and the browser's navigation. The team exists, the student is on it, and the only thing lost is the trip to `/setup`; the navigation carried no state, so the guide itself is unchanged by the detour. The recovery is to open the dashboard, which is where `/connect` sends them from then on.
...
**Discord.** Not involved. A team's Discord channel is chosen after the team exists. The dropped-link notice appears at the top of this screen for a student who was bounced off a Discord or device link (`apps/portal/src/routes/ConnectPage.tsx:88`).
...
- **The fork button opens a new tab and does not come back.** It calls `window.open` with the template's `/fork` page (`apps/portal/src/routes/ConnectPage.tsx:454`). There is no callback and no detection of a completed fork; the student returns to the portal tab and presses "Check again".
...
- **The team name is capped in two places and trimmed in three.** The field's `maxLength` is 60 (`apps/portal/src/routes/ConnectPage.tsx:392`), the contract's max is 60 (`packages/contracts/src/schema.ts:1009`), the generated default is truncated to 60 (`apps/portal/src/routes/ConnectPage.tsx:503`), and a whitespace-only value is normalized to absent before validation (`apps/portal/worker/routes/github.ts:39`). Nothing about a long or blank name can reach the database.
...
- Whether a student who forked into an organization understands that the app must be installed on the organization, not on their personal account, is the highest-value thing to watch in a usability pass. Step 02 says "Install the app on the account that owns the fork." (`apps/portal/src/routes/ConnectPage.tsx:475`), and the screen no longer mentions an organization anywhere, so the phrase "the account that owns the fork" carries the whole idea for a student who has never installed a GitHub App.
