...
| Which week's benchmark | The overview reports completed practice and official usage across benchmarks and versions. It is not a remaining-capacity figure for one benchmark. | No effect. There is no track switcher here. |
...
**Credit.** Staff see completed practice and official usage within their team-access scope. Failures are not counted, and historical refunded successes remain excluded. The overview does not grant credit. See [credit and quota](../cross-cutting/credit-and-quota.md).
...
- **The old refund count no longer reports policy.** Its compatibility value is zero; failure uses no quota without a refund operation.
...
Verified against Cog\*Portal commit `a0e8eac` for usage policy; unchanged layout references retain the earlier draft.
