...
| SETUP-12 | P2 | none | Completion says what it proves and what it does not ([How it ends](../portal/setup.md)). | A guide at N of N. | 1. Read the SETUP COMPLETE panel. | It says every command reported back and that the tool can find and call the work. It does not call it a grade, say the code is good, or claim anything about the machine in front of you: the evidence is stored per user and team, not per device. | not run |
