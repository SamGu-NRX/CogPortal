...
| SETUP-09 | P1 | none | The page says that boxes fill in groups, before a student watches them not fill ([The simple case](../portal/setup.md)). | A guide at 0 of 5. | 1. Read the paragraph above the commands. | It says nothing ticks as you type, that the last command reports back, and that it fills several boxes at once. | not run |
| SETUP-10 | P1 | linked device | One `check` fills four boxes together ([The simple case](../portal/setup.md)). | A guide at 0 of 5 with a linked device, in the team's worktree. | 1. Run `cogworks check --benchmark <id> --update-setup`.<br>2. Watch without reloading. | Clone, tool, benchmark and check all turn green on the same repaint, matching the terminal's `setup: updated clone, environment, project, wiring`. | not run |
| SETUP-11 | P2 | none | The page explains what setup buys before a student leaves it ([How it ends](../portal/setup.md)). | Any guide. | 1. Read the WHAT THIS GETS YOU panel. | Local runs unlimited, hosted practice with the real remaining count, official attempts on the hidden set, and a runnable `cogworks run --benchmark <id>` line. | not run |
| SETUP-12 | P2 | none | Completion says what it proves and what it does not ([How it ends](../portal/setup.md)). | A guide at 5 of 5. | 1. Read the SETUP COMPLETE panel. | It says the machine can find and call the code, points at a local run first, and does not call it a grade or say the code is good. | not run |
...
| START-08 | P2 | any team with usage | The attempt budget says what the cells cannot ([Answered without work](../portal/start-a-practice-run.md)). | A team with at least one run. | 1. Read the ATTEMPT BUDGET panel. | Local runs are unlimited and uncounted, and an official attempt is spent once its run reaches evaluation, so a run refused earlier does not cost one. | not run |
