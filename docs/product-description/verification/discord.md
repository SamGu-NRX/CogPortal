...
| COG-06 | P1 | discord | Hosted verification states its quota consequence ([the confirmations](../discord/commands.md)). | A team with practice capacity. | 1. Choose "Verify hosted" and read the confirmation. | It explains that completed practice evaluations count and failures use no quota. Record the exact text. | not run |
| COG-07 | P1 | discord | Starting an official evaluation is confirmed with its number ([the confirmations](../discord/commands.md)). | A promotable run. | 1. Choose "Promote to official". | The confirm button names which attempt of three. Compare that number with the dashboard. | not run |
