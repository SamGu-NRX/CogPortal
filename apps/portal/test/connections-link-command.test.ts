...
import { deviceLinkCommand, setupCommandLines } from "../src/lib/setup-progress.ts";
...
test("the command names a portal, so an unconfigured CLI can run it", () => {
  const command = deviceLinkCommand(ORIGIN);
...
test("the setup rail reads the same helper the panel does", () => {
  ...
  assert.equal(rail.command, deviceLinkCommand(ORIGIN));
});
