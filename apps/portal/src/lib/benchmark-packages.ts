 ...
 * pickle. It is also the first commit carrying the four repairs a security
 * review blocked the boundary on: a killed run can no longer report
 * `completed`, resource limits are confirmed installed before student code
 * runs, `test` and `run` can no longer hang with no deadline armed, and a
 * repository that declares its own submission no longer needs a warm course
 * cache. Installed into a clean 3.8.20 venv from this exact commit on
 * 2026-09-10 to confirm all four are in the installed source.
 */
