...
export async function enqueueRun(
  ...
  if (stored.dispatchJobJson === null) {
    ...
    if (!stored.preparedArtifactId) {
      // Weights are stored under the repository and commit they were synced
      // for, so the lookup uses the repository the run recorded and falls back
      // to the team's current name only when the run has none. A team renamed
      // or repointed after dispatch must not redirect this lookup.
      //
      // The recorded repository ID travels with it as the provenance key, so a
      // report that matches by name but belongs to a different repository is
      // still refused.
